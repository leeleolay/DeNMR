...
class MolecularVisualization:
    ...
    def visualizeNmr(self, path: str, path_true, molecules: list, molecules_true, num_molecules_to_visualize: int, log='graph'):
        # define path to save figures
        if not os.path.exists(path):
            os.makedirs(path)
        ...
        for i in range(num_molecules_to_visualize):
            ...
            mol = self.mol_from_graphs(molecules[i][0].numpy(), molecules[i][1].numpy())
            # print(f'predict{molecules[i][0].numpy()}')
            # print(f'true{molecules_true[i][0].numpy()}')
            mol_true = self.mol_from_graphs(molecules_true[i][0].numpy(), molecules_true[i][1].numpy())
            try:
                Draw.MolToFile(mol, file_path)
                Draw.MolToFile(mol_true, file_path_true)
                # mol.UpdatePropertyCache(strict=True)  # 强制更新并检查分子信息
                # mol.GetRingInfo()
                # mol_true.UpdatePropertyCache(strict=True)  # 强制更新并检查分子信息
                # mol_true.GetRingInfo()
                # mol.GetRingInfo()
                # mol_true.GetRingInfo()
                # Chem.SanitizeMol(mol)
                # Chem.SanitizeMol(mol_true)
                # fp1 = AllChem.GetMorganFingerprintAsBitVect(mol, 2)  # 2 是半径
                # fp2 = AllChem.GetMorganFingerprintAsBitVect(mol_true, 2)
                # # fp1 = AllChem.GetMorganFingerprint(mol, radius=2)
                # # fp2 = AllChem.GetMorganFingerprint(mol_true, radius=2)
                # # similarity = DataStructs.TanimotoSimilarity(fp1, fp2)
                # # print(f'TanimotoSimilarity of molecular{i}:{similarity:.4f}')
