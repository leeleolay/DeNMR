...
from models.molecularGT import molecularGT
...
class DiscreteDenoisingDiffusionMolecular_condition(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools, extra_features,
        ...
        self.model = molecularGT(n_layers_GT=cfg.model.n_layers,
    ...
    def validation_step(self, data, i):
        ...
        nll = self.compute_val_loss(pred, noisy_data, dense_data.X, dense_data.E, data.y, node_mask, condition=[], test=False)
        return {'loss': nll}
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        #     self.val_data_X = torch.cat(self.val_data_X, dim=0)
        #     print(f'self.val_data_X{self.val_data_X.shape}')
        #     self.val_data_E = torch.cat(self.val_data_E, dim=0)
        #     # self.val_y_collection = self.val_y_collection.view(-1,self.vocabDim)
        #     # self.val_atomCount = self.val_atomCount.view(-1,1)
        #     num_examples = self.val_atomCount.size(0)
        #
        #
        #     start_index = 0
        #
        #     self.val_allnum = len(self.val_data_X)
        #     self.val_right = 0
        #
        ...
        #             self.val_data_X = self.val_data_X.repeat(ratio+1, 1)
        #             self.val_data_E = self.val_data_E.repeat(ratio+1, 1)
        ...
        #         batch_X = self.val_data_X[start_index:start_index + to_generate]
        #         batch_E = self.val_data_E[start_index:start_index + to_generate]
        #
    ...
    def test_step(self, data, i):
        ...
        pred = self.forward(noisy_data, extra_data, node_mask, dense_data.X, dense_data.E)
        ...
        nll = self.compute_val_loss(pred, noisy_data, dense_data.X, dense_data.E, data.y, node_mask, condition=[], test= True)
        return {'loss': nll}
    ...
    def on_test_epoch_end(self) -> None:
        ...
        self.test_data_X = torch.cat(self.test_data_X, dim=0)
        self.test_data_E = torch.cat(self.test_data_E, dim=0)
        # self.test_y_collection = self.test_y_collection.view(-1, self.vocabDim)
        num_examples = self.test_atomCount.size(0)
        start_index = 0

        self.test_allnum = len(self.test_data_X)
        self.test_right = 0

        self.test_right_smiles = 0
        self.zero_vector = torch.zeros(len(self.test_data_X))
        ...
        for repeat in range(1):
            ...
            samples_left_to_generate = len(self.test_data_X)
            samples_left_to_save = len(self.test_data_X)
            chains_left_to_save = len(self.test_data_X)

            # r_test_y_collection = self.test_y_collection.clone()
            r_test_atomCount = self.test_atomCount.clone()
            r_test_x = self.test_data_X.clone()
            r_test_e = self.test_data_E.clone()
            ...
            while samples_left_to_generate > 0:
                ...
                if to_generate > num_examples:
                    ratio = to_generate // num_examples
                    # r_test_y_collection = r_test_y_collection.repeat(ratio + 1, 1)
                ...
                # batch_y = r_test_y_collection[start_index:start_index + to_generate]
                batch_y = []
        ...
        self.test_right_smiles = torch.sum(self.zero_vector)
        self.print(f'right:{self.test_right_smiles}')
        self.print(f'all:{self.test_allnum}')
        self.print(f'accuracy:{self.test_right_smiles / self.test_allnum}')
    ...
    def forward(self, noisy_data, extra_data, node_mask, X, E):
        ...
        # print(f'Xshape{X_.shape}')
        # print(f'node_mask{node_mask.shape}')
        # print(f'X_condition{X.shape}')
        # condition = torch.tensor(condition)
        # 生成 [batch] 大小的随机概率向量，每个元素以 15% 的概率小于 0.15
        # 将需要置为 0 的行设置为 0
        return self.model(X_, E_, y_, node_mask, xtrue, etrue)
